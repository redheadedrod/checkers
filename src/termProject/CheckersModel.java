...
public class CheckersModel {
		...
		if (this.piece.getPlayer() == Player.Red) {
			if (startRow + 1 == endRow && (startCol + 1 == endCol || startCol - 1 == endCol)) {
				...
				if (startCol + 2 == endCol) {
					if (this.board[startRow + 1][startCol + 1] == null) {
						return type;
					} else if (this.board[startRow + 1][startCol + 1].getPlayer() == Player.Black) {
						type[1] = true;
						return type;
					}
			...
			if (startRow - 1 == endRow && (startCol + 1 == endCol || startCol - 1 == endCol)) {
				...
				if (startCol - 2 == endCol) {
					if (this.board[startRow - 1][startCol - 1] == null) {
						return type;
					} else if (this.board[startRow - 1][startCol - 1].getPlayer() == Player.Red) {
						type[1] = true;
						return type;
					}
				} else if (startCol + 2 == endCol) {
		...
		if (this.player == Player.Black) {
			...
			for (int row = 0; row < 8; row++) {
				for (int col = 0; col < 8; col++) {
					if (this.board[row][col] == null) {
						continue;
					} else if (this.board[row][col].getPlayer() == Player.Black) {
						numOfPieces++;
						if (numOfPieces > 1) {
							return false;
						} else {
	 ...
	 */
	public boolean canMoveAnywhere(int pRow, int pCol) {
		for (int row = 0; row < 8; row++) {
			for (int col = 0; col < 8; col++) {
				if (this.canMove(pRow, pCol, row, col)[0] || this.canMove(pRow, pCol, row, col)[1]) {
					return true;
				}
			}
		}
